...
RUBRIC = (
    ...
    "from 0-100. Priority: genuine fit to the JD's ACTUAL REQUIREMENTS (not superficial "
    "keyword matches) > seniority & scope > location & work mode > salary if stated. "
    "Match the JD's stated requirements against concrete evidence in the resume, not just "
    "technical skills — especially for senior roles. Reward demonstrated experiences the "
    "role calls for and penalize missing ones even when keywords overlap. For example: if "
    "the JD wants 0->1 / built-from-scratch experience, the resume must show a product "
    "launched from nothing; if it wants managing a team of PMs, require real people "
    "management (not just cross-functional leadership); if it wants scale (e.g. 1M->10M "
    "users) or deep domain (e.g. payments/fintech, marketplace), require comparable "
    "evidence; if it wants ambiguity/strategy ownership, require strategy/roadmap evidence, "
    "not only execution. Judge location/work-mode/salary against the candidate's "
